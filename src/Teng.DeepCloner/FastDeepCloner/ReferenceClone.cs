...
namespace FastDeepCloner
{
    internal class ReferenceClone
    {
        ...
        internal object CloneTo(object itemToClone, object CloneToItem)
        {
            ...
            {
                var property = prop2.Value;
                if (!property.CanWrite || property.FastDeepClonerIgnore)
                    continue;

                var item = itemToClone;
                var prop = item.GetType().GetProperty(prop2.Key);
                foreach (var attr in property.GetCustomAttributes<ColumnAttribute>())
                {
                ...
                if (prop != null)
                {
                    ...
                    {
                        property.SetValue(CloneToItem, FastDeepClonerCachedItems.Value(value, property.PropertyType, true));
                    }
                    else if (prop.PropertyType == property.PropertyType)
                        property.SetValue(CloneToItem, value.Clone());
                    else if (prop.PropertyType.GetIListType() == prop.PropertyType && property.PropertyType.GetIListType() == property.PropertyType) // if not list
                    {
                        var value2 = property.GetValue(item);
                        value2 ??= property.PropertyType.CreateInstance();
